  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12">
      ...
      <nav className="mb-6 text-sm text-[#8a857c]">
        ...
        <span className="mx-2">/</span>
        <span className="text-[#55514a]">{aName} vs {bName}</span>
      </nav>

      <h1 className="mb-4 text-4xl font-black text-[#1a1a1a] md:text-5xl">
        {aName} vs {bName}
      </h1>
      <p className="mb-10 text-lg text-[#55514a]">
      ...
      <div className="mb-10 grid grid-cols-2 gap-6">
        {[{ review: reviewA, name: aName }, { review: reviewB, name: bName }].map(({ review, name }) => (
          <div
            ...
            className="rounded-2xl border border-[#e6e2da] bg-white p-6"
          >
            <h2 className="mb-3 text-xl font-bold text-[#1a1a1a]">{name}</h2>
            <StarRating rating={review.frontmatter.rating} size="sm" />
            <p className="mt-3 text-sm text-[#55514a]">{review.frontmatter.verdict}</p>
            ...
            <div className="mt-4 space-y-1">
              ...
              {review.frontmatter.pros.slice(0, 3).map((p, i) => (
                <p key={i} className="text-xs text-[#55514a]">✓ {p}</p>
              ))}
            </div>
            <Link
              href={`/reviews/${review.frontmatter.slug}`}
              className="mt-4 inline-block text-xs text-[#b8460f] hover:text-[#b8460f] transition"
            >
              Full review →
            </Link>
          </div>
      ...
      {/* Comparison table */}
      <div className="mb-10 overflow-hidden rounded-xl border border-[#e6e2da]">
        <table className="w-full">
          <thead>
            <tr className="border-b border-[#e6e2da] bg-white">
              <th className="px-5 py-3 text-left text-[#55514a] text-sm">Feature</th>
              <th className="px-5 py-3 text-left text-[#1a1a1a] text-sm">{aName}</th>
              <th className="px-5 py-3 text-left text-[#1a1a1a] text-sm">{bName}</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row) => (
              <tr key={row.label} className="border-b border-[#e6e2da]">
                <td className="px-5 py-3 text-sm text-[#8a857c]">{row.label}</td>
                <td className={`px-5 py-3 text-sm ${row.winner === "a" ? "font-semibold text-[#b8460f]" : "text-[#55514a]"}`}>
                  {row.a}
                </td>
                <td className={`px-5 py-3 text-sm ${row.winner === "b" ? "font-semibold text-[#b8460f]" : "text-[#55514a]"}`}>
                  {row.b}
                </td>
              </tr>
            ))}
      ...
      {/* Verdict */}
      <div className="mb-10 rounded-xl border border-[#e6e2da] bg-white p-6">
        <h2 className="mb-3 text-xl font-bold text-[#1a1a1a]">Our Verdict</h2>
      ...
      {/* Bonfire CTA */}
      <div className="rounded-2xl border border-[#1b3a6b]/40 bg-[#eef1f6] p-8 text-center">

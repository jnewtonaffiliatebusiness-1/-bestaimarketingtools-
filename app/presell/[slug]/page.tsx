  ...
  return (
    <div className="min-h-screen bg-[#f7f6f2] text-[#1a1a1a]">
      <main className="mx-auto max-w-3xl px-5 py-10">
        ...
        <p className="mb-4 text-[10px] uppercase tracking-widest text-[#8a857c]">
        ...
        <img
          ...
          width={1200}
          height={630}
          className="mb-8 w-full rounded-lg border border-[#e6e2da]"
        />

        {/* ── Hook ── */}
        <article>
          <h1 className={`${serif} text-3xl font-bold leading-tight text-[#1a1a1a] md:text-4xl`}>
          ...
          <p className="mt-6 text-lg leading-relaxed text-[#55514a]">
          ...
          <section className="mt-8 rounded-lg border-l-4 border-[#1b3a6b] bg-[#eef1f6] p-6">
            <p className="text-[11px] font-semibold uppercase tracking-widest text-[#1b3a6b]">
              The verdict
            </p>
            <p className={`${serif} mt-2 text-lg leading-relaxed text-[#1a1a1a]`}>{fm.verdict}</p>
            <div className="mt-4 flex flex-wrap gap-x-6 gap-y-2 text-sm text-[#55514a]">
              <span>
                Our rating: <strong className="text-[#1a1a1a]">{fm.rating.toFixed(1)} / 5</strong>
              </span>
              <span>
                Starts at: <strong className="text-[#1a1a1a]">{price}</strong>
              </span>
            </div>
          ...
          {/* ── What it's good at ── */}
          <h2 className={`${serif} mt-10 text-2xl font-bold text-[#1a1a1a]`}>
            What {name} gets right
          </h2>
          <ul className="mt-4 space-y-3">
            {fm.pros.map((p) => (
              <li key={p} className="flex gap-3 text-[#55514a]">
                <span className="mt-1 flex-shrink-0 text-[#2f5d50]">✓</span>
                <span>{p}</span>
              </li>
            ))}
          </ul>
          ...
          <ul className="mt-4 space-y-3">
            {fm.cons.map((c) => (
              <li key={c} className="flex gap-3 text-[#55514a]">
                <span className="mt-1 flex-shrink-0 text-[#b8460f]">✗</span>
                <span>{c}</span>
              </li>
            ))}
          </ul>
          ...
          <p className="mt-4 leading-relaxed text-[#55514a]">
          ...
          {/* ── CTA ── */}
          <div className="mt-8 rounded-lg border border-[#e6e2da] bg-white p-6 text-center">
            <a
              href={offerUrl}
              rel="sponsored nofollow noopener"
              className="inline-flex items-center justify-center rounded-md bg-[#b8460f] px-8 py-3.5 text-base font-bold text-white transition-transform hover:scale-105"
            >
              Try {name} →
            </a>
            <p className="mt-3 text-xs text-[#8a857c]">
          ...
          <p className="mt-8 text-center text-sm text-[#55514a]">
            Want the full breakdown — pricing tiers, feature list, alternatives?{" "}
            <Link
              href={`/reviews/${params.slug}`}
              className="text-[#1b3a6b] underline underline-offset-4 hover:text-[#b8460f]"
            >
        ...
        <footer className="mt-12 border-t border-[#e6e2da] pt-6 text-xs leading-relaxed text-[#8a857c]">
          <p>
